...
public final class ArgumentFactory {
    ...
    final static Option ENDPOINT = new Option("e", true, "The ds3 endpoint to connect to or have \"DS3_ENDPOINT\" set as an environment variable.");
    final static Option ACCESS_KEY = new Option("a", true, "Access Key ID or have \"DS3_ACCESS_KEY\" set as an environment variable");
    final static Option SECRET_KEY = new Option("k", true, "Secret access key or have \"DS3_SECRET_KEY\" set as an environment variable");
    final static Option COMMAND = new Option("c", true, "The Command to execute.  For Possible values, use '--help list_commands.'" );
    final static Option PROXY = new Option("x", true, "The URL of the PROXY server to use or have \"http_proxy\" set as an environment variable");
    final static Option HTTP = Option.builder().longOpt("http").desc("Send all requests over standard HTTP").build();
    final static Option INSECURE = Option.builder().longOpt("insecure").desc("Ignore ssl certificate verification").build();
    final static Option PRINT_HELP = new Option("h", "Help Menu");
    final static Option COMMAND_HELP = Option.builder()
            .longOpt("help")
            .desc("Command Help (provide command name from -c)")
            .optionalArg(true)
            .numberOfArgs(1)
            .build();
    static final Option VERBOSE = Option.builder().longOpt("verbose").desc("Log output to console.").build();
    static final Option DEBUG = Option.builder().longOpt("debug").desc("Debug (more verbose) output to console.").build();
    static final Option TRACE = Option.builder().longOpt("trace").desc("Trace (most verbose) output to console.").build();
    static final Option LOG_VERBOSE = Option.builder().longOpt("log-verbose").desc("Log output to log file.").build();
    static final Option LOG_DEBUG = Option.builder().longOpt("log-debug").desc("Debug (more verbose) output to log file.").build();
    static final Option LOG_TRACE = Option.builder().longOpt("log-trace").desc("Trace (most verbose) output to log file.").build();
    static final Option PRINT_VERSION = Option.builder().longOpt("version").desc("Print version information").build();
    static final Option RETRIES = new Option("r", true,
            "Specifies how many times puts and gets will be attempted before failing the request. The default is 5");
    static final Option BUFFER_SIZE = new Option("bs", true, "Set the buffer size in bytes. The default is 1MB");
    ...
    public final static Option ALL_VERSIONS = Option.builder().argName("all-versions").longOpt("all-versions").build();
    public static final Option VERSION_ID = Option.builder("vid").longOpt("version-id").desc("Version ID value").hasArg(true)
            .build();
    ...
    static final Option NO_FOLLOW_SYMLINKS = Option.builder().longOpt("follow-symlinks")
    ...
    static final Option VIEW_TYPE = Option.builder().longOpt("output-format").hasArg()
